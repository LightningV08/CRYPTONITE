            ...
            if (checkPasswordResult.isEmpty() || buttonClickedInRockyou) {
                executor.execute(new AsyncExecutor.AsyncExecutable() {
                    ...
                    public void doInUIThread() {
                        if (finished) {
                            if (success) {
                                Toast.makeText(RC4EncryptActivity.this, R.string.file_encrypted, Toast.LENGTH_SHORT).show();
                                setResult(RESULT_OK, getIntent());
                            } else
                                Toast.makeText(RC4EncryptActivity.this, R.string.encryption_error, Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            } else {
